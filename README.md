...
## 🔎 `make check-contract` — 조용히 죽는 것을 매일 잡습니다

이 프로젝트에서 제일 비싼 사고는 **에러가 안 나는 사고**입니다.

- 노드 SG 에 태그 하나가 빠지면 파드가 DB 에 못 붙습니다. 증상은 **연결 타임아웃뿐**입니다.
- IRSA 역할의 ServiceAccount 가 한 글자 어긋나면 **'권한 없음'** 만 뜹니다. IAM 정책은 멀쩡한데 거기를 파게 됩니다.

이런 것들을 **시연 직전이 아니라 매일** 잡자는 게 이 검증기입니다. 기준은 **규약서(`docs/네이밍규약서.md`)** 입니다.

### 2단으로 돕니다

| 단계 | 무엇을 보나 | 자격증명 | 언제 |
|---|---|---|---|
| **정적** | infra 의 **terraform 소스** | 불필요 | **지금부터** |
...
**이 두 가지를 안 지키면 검증기가 조용히 무력해집니다.**

- **블록을 봅니다.** "파일 어딘가에 태그가 있다" 로 통과시키면, 그 태그가 **노드 SG 에** 붙었는지 알 수 없습니다. 런치템플릿에 붙어 있어도 초록불이 뜹니다. 그래서 중괄호를 세어 리소스 블록을 꺼내고 **그 안에서** 찾습니다. SG 는 이름(`hailcast-dev-sg-eks-node`)으로 특정합니다.
- **값을 봅니다.** 두 태그가 서로 같은지만 보면 **둘 다 `hailcast-prod` 여도 통과**합니다. `local.name_prefix` 를 `hailcast-dev` 라고 **가정하지 않고** 그 모듈의 `locals` 를 실제로 읽어 풉니다. 그래야 리터럴로 쓰든 `${var.project_name}-${var.environment}` 로 쓰든 같은 값이면 같다고 봅니다.

### 빨간불이 뜨면

**대부분 apply 하면 조용히 죽는 것들입니다.** 화면에 `파일:줄`과 규약서 절 번호(§N)가 함께 찍히니 그걸 따라가면 됩니다.

**규약이 틀렸다고 판단되면 코드가 아니라 규약을 고칩니다.** 그때는 **infra(문서) + ops(검증기) 짝 PR** 입니다.
검증기의 기대값은 규약서의 사본이라, 한쪽만 고치면 갈라집니다. 다만 갈라지면 **이 검증기가 빨간불로 알려 줍니다.**
...
> 낡은 브랜치를 체크아웃해 둔 채 돌리면 낡은 코드를 검사합니다. 그래서 화면 맨 위에 **어느 브랜치·커밋을 읽었는지** 찍습니다. 통합 상태를 보려면 `dev` 에서 돌리세요.
